...
<?php  echo "<h3>You are   {$sp7ffffe} :"; echo "<h3>You have requested results for Jotto player {$spf97a4b} :"; $sp5b2588 = mysql_connect('localhost', 'root', 'vicnum'); mysql_select_db('vicnum', $sp5b2588); $sp1cf139 = 'SELECT * FROM jottoresults '; $sp3c366f = mysql_query($sp1cf139) or die("ERROR in {$sp1cf139}" . ' ' . mysql_error()); $spb2dc76 = mysql_num_rows($sp3c366f); $sp1cf139 = "SELECT name,guess,count,tod FROM jottoresults WHERE name  = '{$spf97a4b}'"; $sp85efff = mysql_query($sp1cf139) or die("ERROR in {$sp1cf139}" . ' ' . mysql_error()); if (mysql_num_rows($sp85efff) > $spb2dc76) { $sp1cf139 = "INSERT INTO unionresults(name,unionquery) VALUES(\"{$sp7ffffe}\",\"{$spf97a4b}\")"; $sp95ad53 = mysql_query($sp1cf139) or die("ERROR in {$sp1cf139}" . ' ' . mysql_error()); } if (mysql_num_rows($sp85efff) > 0) { while ($spa05ab6 = mysql_fetch_array($sp85efff, MYSQL_NUM)) { print '
